...
import { MatchVerdict } from "@/components/generate/match-verdict";
import {
  ChainOfThought,
...
type Phase = "form" | "captured" | "preflight" | "verdict" | "starting" | "streaming";
...
const PHASES = [
  ...
  },
  {
    ...
    specialists: ["voice_fingerprint_extractor", "honesty_calibrator", "credibility_scanner"],
  },
  {
    key: "matching",
...
function NewGenerationInner() {
  ...
  // The JD the user actually submitted — kept in state so the post-verdict
  // launch doesn't depend on URL prefill params (which are empty when the
  // JD was pasted by hand).
  const [jdPayload, setJdPayload] = useState<{
    mode: "text" | "url";
    jdText?: string;
    jdUrl?: string;
    market: "us" | "uk";
  ...
  const { start: startStream, status, traceEntries, currentSpecialist, completionData } =
    useGenerationStream();
  ...
  useEffect(() => {
    fetch("/api/profile")
      ...
      .then((d) => {
        if (d?.fullName) setFirstName(d.fullName.split(" ")[0]);
      })
      .catch(() => {});
  }, []);
  ...
  async function start(payload: {
    ...
    try {
      ...
      const preflightData = (await preflightRes
        .json()
        .catch(() => null)) as PreflightDetectResponse | null;
      if (!preflightRes.ok || !preflightData)
        throw new Error("Failed to run preflight drift detection.");

      // Verdict-first: ALWAYS show the honest match read before any
      // credits are spent. Generation only starts on an explicit click.
  ...
  async function continueAfterDrift(answers: DriftAnswer[]) {
    ...
    try {
      ...
      if (!save.ok || !hasPreflightToken(saveData)) {
        throw new Error(
          (typeof saveData === "object" && saveData && "error" in saveData
            ? saveData.error
            : undefined) || "Could not save drift answers.",
        );
      }
      const profile = await fetch("/api/profile").then((r) => r.json().catch(() => null));
      // Prefer the JD captured in state this session; fall back to URL
      // prefill params (deep-link entry).
      await launchGeneration({
        jdUrl: jdPayload?.mode === "url" ? jdPayload.jdUrl : prefilledUrl || undefined,
        jdText: jdPayload?.mode === "text" ? jdPayload.jdText : prefilledJd || undefined,
        profileMarkdown: profile?.profileMarkdown ?? "",
        market: (jdPayload?.market ?? prefilledMarket).toUpperCase(),
  ...
  function cancelVerdict() {
    setPreflight(null);
    setPreflightJdHash(null);
    setJdPayload(null);
    setError("");
    setPhase("form");
  }

  const busy = phase !== "form" && phase !== "verdict";
  const isStreaming = phase === "streaming";
  const showVerdict = phase === "verdict" && preflight !== null;
  ...
  return (
    <PageShell width="wide" className="flex min-h-[calc(100vh-4rem)] flex-col">
      <PageHeader
        ...
        subtitle="Paste a job URL or the full description. We'll check your profile fits the role, then write your package."
      />
      ...
      {busy ? (
        <ChainOfThought className="mt-2">
          <ChainOfThoughtStep defaultOpen={phase === "captured"}>
            <ChainOfThoughtTrigger>
              {phase === "captured" ? (
                <TextShimmer>Capturing the job description</TextShimmer>
              ) : (
          ...
          <ChainOfThoughtStep defaultOpen={phase === "preflight"}>
            <ChainOfThoughtTrigger>
              {phase === "preflight" ? (
                <TextShimmer>Checking profile drift</TextShimmer>
              ) : (
          ...
          <ChainOfThoughtStep defaultOpen={phase === "starting" || isStreaming}>
            <ChainOfThoughtTrigger>
              {phase === "starting" || isStreaming ? (
                <TextShimmer>Starting the tuning workflow</TextShimmer>
              ) : (
                  ...
                  {PHASES.map((p) => {
                    ...
                    return (
                      <ChainOfThoughtStep key={p.key} defaultOpen={s === "active"}>
                        ...
                        <ChainOfThoughtContent>
                          {traceEntries
                            .filter((t) => p.specialists.includes(t.specialist))
                            .map((t) => (
                              <ChainOfThoughtItem key={t.seq}>
                                <span className="font-mono text-[11px] text-muted-foreground/60">
                                  {humanize(t.specialist)}
                                </span>
        ...
        <JdPrompt
          ...
          busy={busy}
          placeholderText={
            prefilledJd || "Paste a job description, a job URL, or describe the role…"
          }
        />
      )}
      ...
      {showVerdict && preflight ? (
        <>
          <MatchVerdict
            preflight={preflight}
            busy={driftSaving}
            showGenerateCta={preflight.questions.length === 0}
            onGenerate={() => void continueAfterDrift([])}
            onCancel={cancelVerdict}
          />
      ...
      <div className="mt-auto pt-16 flex flex-wrap gap-6 text-[11px] text-muted-foreground/50 [&_span]:text-muted-foreground/50 [&_button:hover_span]:text-muted-foreground [&_svg]:size-3">
        ...
        <Reasoning>
          ...
          <ReasoningContent contentClassName="text-[11px] text-muted-foreground/50 prose-none not-prose">
            Retuned may ask a few drift questions if the role needs evidence not yet in your career
            profile.
          </ReasoningContent>
        </Reasoning>
      </div>
